...
> **状态**：已被 `P1-S03C_CONTINUOUS_NATION_SPEC.md` 取代；其中日/周纯函数、存档迁移和玩家语言映射可作为实现参考，但“期末自动暂停/确认下期”的规则不得继续使用。
...
首局生活底线另用固定的每日需求，不从计划期资源账的 `demand` 字段派生：可饮用水 **2 单位/日**、食物储备 **2 单位/日**。因此初始 120 水与 100 食物分别约为 60 日与 50 日；资源账继续负责计划期的净收支。
