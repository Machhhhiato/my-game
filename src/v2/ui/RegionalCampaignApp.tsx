...
export function RegionalCampaignApp({ initialState, onRestart }: { initialState: NationKernelState; onRestart: () => void }) {
  const [state, setState] = useState<NationKernelState>(() => installRegionalCampaignContent(loadRegionalState(initialState)));
  ...
  const operations = useMemo(() => REGIONAL_OPERATION_DEFINITIONS.map((definition) => ({ definition, operation: state.operations[definition.id] })), [state]);
  ...
  return <main className="text-idle-shell">
    <header className="text-idle-topbar">
      <div><span className="text-idle-kicker">大统合联邦 · 文字战役</span><h1>区域网络</h1></div>
      <div className="text-idle-status"><strong>余烬历 {state.calendar.year} 年 {state.calendar.month} 月 · {state.calendar.phase === 'early' ? '上旬' : state.calendar.phase === 'mid' ? '中旬' : '下旬'}</strong><span>单一共同体 · {player.population.residents} 人登记在册 · 可调度 {availableWorkforce(state, state.playerPolityId)} 人</span></div>
    ...
    <section className="text-idle-alert ready" aria-live="polite"><div><strong>{integrationComplete ? '区域统合已经完成' : complete ? '区域网络已经形成' : '把临时外拓变成稳定网络'}</strong><span>{integrationComplete ? '多个内部节点、人口调拨和服务班次已经进入共同运行程序。' : complete ? '中继点、补给线和共同规则已接入同一套区域运行结构；现在开始建立内部节点与服务统合。' : notice}</span><small>区域服务 {region?.integration.services ?? 0} · 执行能力 {region?.integration.executionQuality ?? 0} · 建设物资 {(state.quantities[`polity:${state.playerPolityId}`]?.['construction.ldu']?.current ?? 0).toFixed(1)}</small></div><p><b>阶段目标：</b>{complete ? '建立多个内部节点，调拨常驻人口与公共服务班次，再完成内部统合章程。' : '先勘察，再建设中继点和补给线；所有结果都写入国家内核，供统合阶段继续使用。'}</p></section>
    <section className="text-idle-grid regional-grid">
      <article className="text-idle-card activity-card"><div className="card-heading"><div><span>区域态势</span><h2>{complete ? '固定联接已建立' : '从聚居地向区域延伸'}</h2></div><small>持续运行</small></div><svg className="activity-sketch explore" viewBox="0 0 240 130" aria-hidden="true"><path className="ground" d="M12 105H228" /><path className="route" d="M48 95C90 70 120 84 174 49" /><circle className="marker" cx="176" cy="48" r="9" /><circle className="marker" cx="65" cy="93" r="7" /><path className="person" d="M109 76v20m-9-9h18m-12 9-5 10m9-10 6 10" /></svg><p className="activity-caption">已投用设施 {Object.keys(state.facilities).length} 项 · 已接通网络 {Object.keys(state.networks).length} 条 · 已归档能力 {Object.keys(player.capabilities).length} 项</p></article>
      <article className="text-idle-card work-card regional-operations"><div className="card-heading"><div><span>区域事项</span><h2>按前置关系推进</h2></div><small>{operations.filter(({ operation }) => operation?.status === 'completed').length} / {operations.length} 已完成</small></div><div className="choice-list">{operations.map(({ definition, operation }) => operation == null ? null : <section key={definition.id} className="regional-operation"><div><strong>{kindName(definition.kind)} · {definition.title}</strong><small>{statusName(operation.status)}</small></div><p>{definition.summary}</p><em>完成后：{definition.outcome}</em>{operation.status === 'active' && <><div className="work-meter"><i style={{ width: `${progress(operation)}%`, transitionDuration: `${speed === 0 ? 0 : Math.max(250, BASE_DAY_INTERVAL_MS / speed - 160)}ms` }} /></div><small>{progress(operation)} · 作业人员 {operation.staffRequired} 人</small></>}{operation.status === 'planned' && <button onClick={() => begin(definition.id)}>启动{definition.kind === 'policy' ? '政策' : '事项'}</button>}</section>)}</div></article>
      ...
      <article className="text-idle-card archive-card"><div className="card-heading"><div><span>区域档案</span><h2>已形成的长期事实</h2></div><small>可供下一阶段引用</small></div><div className="archive-columns"><section><strong>设施</strong>{Object.keys(state.facilities).map((id) => <span key={id}>{id === 'facility.regional-relay' ? '区域中继点' : '既有聚居设施'}</span>)}</section><section><strong>网络</strong>{Object.keys(state.networks).length === 0 ? <span>尚无固定网络</span> : Object.keys(state.networks).map((id) => <span key={id}>{id === 'network.regional-supply-route' ? '区域补给线' : id}</span>)}</section><section><strong>阶段边界</strong><span>尚未出现外部政体或战争。</span><span>区域统合将先处理内部节点、服务与协调。</span></section></div></article>
    </section>
  </main>;
}
