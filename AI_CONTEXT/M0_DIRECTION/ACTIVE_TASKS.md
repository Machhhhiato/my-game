...
overall_current_task: M0-L4-402-R2
handoff_id: M0-H003
...
| 第四层（代码） | `M0-L4-402-R2` 正式界面交互骨架修订 | `implemented_waiting_user_recheck` | 用户已接受交互方向；实现通过验证但第一层直接改代码构成流程错误。后续修改必须先派发第四层；不进入 `M0-L4-403` | `WAITING_RECHECK` |
...
- 依赖证据：`M0-L4-010` accepted；用户接受 M0 唯一主线、旧存档不兼容和旧 R38 拟删除基线；`overall_spec_frozen: true`。本条审计发生时 `implementation_authorized: false`；现已由 `D-M0-DIR-037` 更新。
...
- 线程规则修订：用户在 `M0-S003-U136` 明确只有创建动作本身报错才适用“只新建一次”；创建成功后的能力失败允许清理后换用更合适模型接替。第一至第三层默认复用长期会话，第四层可按大型实施批次定期接替，形成 `D-M0-DIR-040`。
- 401-R1 首次派发：`/root/m0_l4_401_r1` 使用 `gpt-5.6-terra / medium` 成功创建，但压缩重写被否决，还原后又确认无法安全完成核心迁移；已按能力失败关闭，工作区恢复输入提交，无提交、无推送。
- 401-R1 接替结果：规则提交 `1caa7a7b667fbab1bbcefb73f5a0eddff54af6a2` 已推送并与远端一致；接替任务 `/root/m0_l4_401_r1_sol` 使用 `gpt-5.6-sol / high` 完成公历、月结、存档、正式 UI 和测试迁移。第一层两轮退回后补修指针拖动，类型检查、M0 测试、构建、差异检查、桌面与窄屏浏览器验收全部通过。
- 401-R1 试玩接受：用户在 `M0-S003-U137` 结束试玩并确认框架大体验收通过。资源提示精简为收入/来源与支出/去向，人力档位转为运行调度令；字体和最终视觉留待内容增加后统一调整。自动验证通过且未重新启动页面，形成 `D-M0-PROD-058` 与 `D-M0-DIR-042`。
- 401 收口与 402 派发：接受提交 `a3a11d76822a9149d2e6862c01a124747d25b03d` 已通过本机 `7897` 代理推送并与远端一致。执行单 `NIGHT_WORK_ORDER_M0_L4_402.md` 已冻结；新第四层任务 `/root/m0_l4_402` 使用 `gpt-5.6-terra / medium`，状态 active，不启动试玩页面。
- 402 首次退回与接替：`/root/m0_l4_402` 的自动测试通过，但领域自动科研、勘测优先级/最大天数和完整地点事实 UI 未实现，且两份冻结规格未完成 EOF 回执，不能标记交付完成。首次实例已停止，白名单内草稿保留供审计；按 `D-M0-DIR-040` 由 `/root/m0_l4_402_sol` 使用 `gpt-5.6-sol / high` 接替同一执行单；这一条的 `active` 只记录当时过程，当前状态以下一条收口记录为准。
- 402-R1 现场修订：用户真实试玩后指出六边格浮动覆盖、科研人数不应直接选择以及资源栏过高。第一层已改为共边 SVG 球面、研究设施岗位与实际入职速度、v5 严格存档和单行资源缩略栏。自动验证与真实浏览器验证均通过，随后由 R2 界面骨架修订接替。
- 402-R2 界面骨架：用户接受“建筑岗位、科技路径、按地块建造和按整厂分配生产”的交互方向，并补充资源必须显示名称、建造选址与既有设施岗位不得放在同一 sheet、当前两项科技只能称科技路径骨架而非完整科技树。第一层已据此拆分总部、科研、探索、工程、生产与日志，加入设施开岗、科技前置自动排队、工程选址、整厂生产线和事实日志；未新增 403 内容。当前实现等待用户复查，科技数量、内容密度与最终视觉均明确留待后续扩充。
- 402-R2 流程纠正：上述产品修改没有先分发第四层，违反 `D-M0-DIR-020` 和第一层角色卡。用户要求在切换新第一层对话前修正规则。提交 `4a34c55` 仍是实现事实，但“第一层直接补修”不再视为合规模式；后续任何产品改动必须先登记并派发职责正确的下层任务。完整用户原话见 `sessions/20260826-1900-mac-m0-s003-amendment-002.md`。
...
- 冻结边界：`overall_spec_frozen: true`，`implementation_authorized: true`；`CONTENT-M0-STORY-001` 已被用户后续修订取代；`TEXT-M0-STORY-001` 仅为 `accepted_for_m0_process_prototype`；`M0-L3-301` 与 `M0-L4-401-R1` accepted；当前 `M0-L4-402-R2` 为 `implemented_waiting_user_recheck`。
...
不同时开启两个生产任务。第一层是唯一总控入口，也是逻辑主线程；第一至第三层默认复用已有长期会话，第四层可按大型实施批次、上下文压力或能力不匹配定期建立接替任务。任务创建动作报错时只新建一次；创建成功后的能力失败可以在清理失败实例后换用更合适模型接替。第一层负责驱动、等待、检查、退回和收回；各线程只回传产物、证据和阻塞，由主线程更新本文件才算正式切换。项目级授权不允许跨越已接受执行单的依赖、白名单和非目标。
...
| `implemented_waiting_user_acceptance` | 实现与第一层相称验证完成，等待用户试玩或内容接受；不得自动推进下游 |
