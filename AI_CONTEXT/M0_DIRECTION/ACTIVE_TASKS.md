...
| 层级 | 当前任务 | 状态 | 阻塞项 | 正确回执 |
|---|---|---|---|---|
| 第一层 | `M0-L1-106` 工程与科技描述结构、风格与总规格冻结 | `active` | 无；前四项 accepted，第五项等待用户复核 | `ACTIVE_REVIEW` |
...
`SPEC-M0-INTEGRATED-001` 与 `M0-L1-105` 已在 `ab02d36` accepted。`M0-L1-106` 第一项至第四项已 accepted；第五项提交 active_review，以谁在说、什么算成功、什么没被算进去三个问题约束第二行，并用具体制度动作去重。`D-M0-DIR-009` 继续要求下游全文读取权威源包并回传来源回执。`SPEC-M0-DESCRIPTION-001` 继续 active，第二、三、四层继续按既定依赖阻塞。
...
- 当前问题：`Q-M0-032` 为 `active_in_M0-L1-106`；第一项至第四项 accepted；第五项“制度性荒诞怎样写、反转怎样去重” active_review，其余三项 pending。
...
- 第五项提案：说话者必须真心认为流程正常；一条只使用一个主要荒诞来源。换机构名不算去重，遮住科技名、机构名和受影响者后仍可套用，或只能概括成`上面得利、下面遭殃`的句子直接重写。
- 冻结边界：`overall_spec_frozen` 仍为 false；`M0-L4-010`、`M0-L2-201`、`M0-L3-301` 继续 blocked_upstream。
- 路由决定：`D-M0-DIR-007`、`D-M0-DIR-009`、`D-M0-PROD-036` 至 `D-M0-PROD-040`。
