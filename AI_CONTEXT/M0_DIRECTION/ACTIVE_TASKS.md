...
| 第四层（代码） | `M0-L4-402` 地图、选址、科研与高级资产 | `authorized_work_order_pending` | 先提交推送 401 收口并冻结 402 执行单；不进入 `M0-L4-403` | `AUTHORIZED_WORK_ORDER_PENDING` |
...
- 冻结边界：`overall_spec_frozen: true`，`implementation_authorized: true`；`CONTENT-M0-STORY-001` 已被用户后续修订取代；`TEXT-M0-STORY-001` 仅为 `accepted_for_m0_process_prototype`；`M0-L3-301` 与 `M0-L4-401-R1` accepted；当前 `M0-L4-402` 为 `authorized_work_order_pending`。
