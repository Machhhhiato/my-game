...
| 第四层（代码） | `M0-L4-402` 地图、选址、科研与高级资产 | `active` | 按 `NIGHT_WORK_ORDER_M0_L4_402.md` 实现；不进入 `M0-L4-403` | `ACTIVE` |
...
- 402 首次退回与接替：`/root/m0_l4_402` 的自动测试通过，但领域自动科研、勘测优先级/最大天数和完整地点事实 UI 未实现，且两份冻结规格未完成 EOF 回执，不能标记交付完成。首次实例已停止，白名单内草稿保留供审计；按 `D-M0-DIR-040` 由 `/root/m0_l4_402_sol` 使用 `gpt-5.6-sol / high` 接替同一执行单，状态 active。
...
- 冻结边界：`overall_spec_frozen: true`，`implementation_authorized: true`；`CONTENT-M0-STORY-001` 已被用户后续修订取代；`TEXT-M0-STORY-001` 仅为 `accepted_for_m0_process_prototype`；`M0-L3-301` 与 `M0-L4-401-R1` accepted；当前 `M0-L4-402` active。
