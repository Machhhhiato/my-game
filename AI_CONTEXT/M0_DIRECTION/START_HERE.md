...
`context/m0-direction` 已在项目根 `AGENTS.md` 安装最短项目内路由。从真实仓库根目录新建任务时，可以只说“你是第 N 层”。正常制作由第一层直接统筹并派发第二至第四层，用户不需要手工复制提示词；这些启动语继续作为手动恢复和诊断入口。从任意其他项目目录自动寻找 Always Game 的全局路由仍未安装，也不是当前 Mac 制作的前置条件。
