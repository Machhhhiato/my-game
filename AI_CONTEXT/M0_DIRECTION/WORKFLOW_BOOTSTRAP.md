...
- 第一层按 `ACTIVE_TASKS.md` 的串行依赖创建、驱动、检查、退回和收回第二至第四层任务，不再增加第五层管理会话。“直接统筹”只表示直接管理线程，不表示直接执行下层工作。
- 各层仍必须读取自己的角色卡和任务卡，只能交付本层产物；第一层不能亲自冒充第二层事件、第三层正文或第四层实现。
- 下层结果统一回到第一层，由第一层核对来源、证据和接受状态并维护 Git 路由。
- 第一层统筹不等于下层已经启动，也不等于自动获得代码写入权。实现必须有可追溯用户授权；现有项目内持续授权可以覆盖其明确范围，不需要重复询问，但任务卡、线程派发、白名单和阶段门仍然缺一不可。
...
3. 阅读本文件。
4. 完整读取 `LAYER_AND_LANGUAGE_RULES.md`。
5. 阅读 `NEXT_ACTION.md`，确认下一目的地、设备、层级和任务。
6. 阅读 `SESSION_ROUTING.md`，确认当前是活动、交接等待还是阻塞。
7. 阅读 `ACTIVE_TASKS.md`，只选择该层当前任务。
8. 阅读所选层级的角色文件。
...
- `routing_state: handoff_ready` 且启动者为第一层：返回 `READY`，显示 handoff ID、待建立 session 和必读原话；启动语本身不修改产品。开始下一轮实质讨论前，先登记新 session 并把路由切回 `active`。
