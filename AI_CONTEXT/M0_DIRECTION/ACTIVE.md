...
`M0-L1-102 · 人力、资源、库存底线与压力`：位于 Mac 的 `M0-DIR-A` / `M0-L1-DIRECTION` / `M0-S003`，当前状态为 `active`。第一项人力方案已接受；当前复核第二项六种库存、首座前哨前启动来源和防软锁规则。
...
等待用户接受或修订第二项资源流方案，并决定 M0 是否需要独立能源或燃料库存。
