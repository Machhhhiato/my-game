...
- 当前动作：等待用户复核第五项提案：去重不是换机构名，而是换荒诞发生的地方；第二行只写一件具体制度事实，不解释笑点。
...
当前项：第五项｜制度性荒诞怎样写、反转怎样去重｜active_review
...
当前提案：谁在说｜什么算成功｜什么没被算进去｜一条只用一种制度动作
下一步：用户接受或修正第五项；接受后再进入第六项
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项信息结构、第二项两行正文、第三项二态展示和第四项最小通知分工 accepted，视觉仍待整体 UI 重做；第五项 active_review。全文源包门禁立即约束未来第二、第三层。第五至第八项逐项接受并最终整体接受后，才可标记 `overall_spec_frozen`。
