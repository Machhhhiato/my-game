...
- 当前动作：用户直接在 `http://127.0.0.1:5173/` 复查 402-R2。重点核对资源名称、总部建筑岗位、科研路径自动排队、工程选址与既有设施岗位分离、探索归档、整厂生产和日志降噪。当前科技只有两项，属于可验收的交互骨架，不得称为内容完整的科技树。未获得实现复查接受前不得进入 `M0-L4-403`。
