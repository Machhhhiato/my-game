...
- 当前动作：第八项第四部分已由 `M0-S003-U083` 接受。`A199` 提出第五部分：接受后第八项整体完成，第一层交付八项复核；只有用户再明确回复最终冻结短语，才设置 `overall_spec_frozen: true` 并启动只读能力审计。
...
当前项：第八项｜动态字段、分层交接、逐条验收与最终冻结｜active
...
当前提案：第八项第五部分｜先结束第八项并复核｜再单独确认冻结｜等待接受
未启动：M0-L4-005｜M0-L4-010｜M0-L2-201｜M0-L3-301｜本轮没有创建下层任务
下一步：等待用户回复 接受第八项第五部分｜随后交付八项完整复核
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项至第七项 accepted，视觉仍待整体 UI 重做；第八项第一至第四部分已接受，第五部分 active_review。revision 6 全文源包门禁立即约束未来第二、第三层。第八项完成后仍须用户单独确认最终冻结，才可标记 `overall_spec_frozen`；冻结后先由第一层派发 `M0-L4-005`。第一层直接统筹不改变冻结门禁，能力审计也不产生工具安装或实施授权。
