...
- 当前动作：让用户复核《恢复精密制造》的两行成品：第一行群星式科技内容，第二行环日式世界内评价。
...
当前项：第二项｜技术内容最低必须写到哪里｜active_review
已撤回：把解锁、条件、阻断和备用方案解释写进科技正文
未接受旧稿：两句可信技术｜伤亡计入人力损耗｜设备按零故障验收
已确认要求：第一行群星式科技内容｜第二行环日式评价
当前候选：恢复精密制造｜统一精度标准与稳定复制｜生产恢复委员会评价
下一步：用户判断两行组合是否正确；正确后再抽取第二项最低标准
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项信息结构 accepted，但视觉明确待整体 UI 重做；第二项 active_review。两行组合要求已经明确，具体成品仍待用户判断。确认正文身份与最低标准后，仍需逐项接受数值状态、文体、强度、长度、动态字段与分层交接，并最终整体接受，才可标记 `overall_spec_frozen`。
