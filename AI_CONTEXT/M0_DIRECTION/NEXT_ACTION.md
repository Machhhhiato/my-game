...
- 当前动作：第二项返工暂停直接改稿；先让用户复核 15 项《群星》与相邻聊天 13 项环日文明现有描述语料的范围和偏好。
...
当前项：第二项｜技术内容最低必须写到哪里｜active_rework
已撤回：把解锁、条件、阻断和备用方案解释写进科技正文
未接受旧稿：两句可信技术｜伤亡计入人力损耗｜设备按零故障验收
当前语料：REF-M0-TECH-DESCRIPTION-CORPUS-001｜群星 15 项｜环日现有 13 项
下一步：用户确认语料范围并指出喜欢与不喜欢的样本；随后再比较和重写正文
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项信息结构 accepted，但视觉明确待整体 UI 重做；第二项 active_rework。当前先确认 28 项现有语料和用户偏好，不把收集动作当成风格接受。完成比较并确认正文身份与最低标准后，仍需逐项接受数值状态、文体、强度、长度、动态字段与分层交接，并最终整体接受，才可标记 `overall_spec_frozen`。
