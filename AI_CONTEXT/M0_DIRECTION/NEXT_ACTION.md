...
- 当前动作：第二项讨论；确定技术正文的共同最低内容和按科研、工程、设备、事故维护、测试验收分别补充的必要内容。
...
当前项：第二项｜技术内容最低必须写到哪里｜active
共同底线：现在为什么做不到｜这次实际解决什么｜完成后仍受什么实体限制
下一步：用户接受第二项或指出三项底线、类型补充、故障/备用边界中的具体修改
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项信息结构 accepted，但视觉明确待整体 UI 重做；第二项 active。之后仍需逐项接受技术内容、数值状态、文体、强度、长度、动态字段与分层交接，并最终整体接受，才可标记 `overall_spec_frozen`。
