...
action_status: ready_waiting_authorization
...
- 当前任务：`M0-L4-401`；依赖已经满足，等待用户针对本任务单独授权实施。
...
- 当前授权：允许维护和推送 M0 方向记录；没有 `M0-L4-401` 游戏实现授权。
- 当前动作：第一层先展示 `M0-L4-401` 有限实施方案、前置检查、夜间检查点和验收方式；用户明确授权后才创建第四层执行线程。
...
当前任务：M0-L4-401｜基础运行、资源、人力与存档｜等待单独授权
任务状态：M0-L3-301 accepted｜M0-L4-401 ready_waiting_authorization
...
同步证据：26ad2ba｜origin/context/m0-direction 已核对一致
下一步：第一层提交 M0-L4-401 实施方案，等待用户单独授权
```

## 完成条件

上一轮完成条件已经满足并在 `26ad2ba` 推送。当前完成条件是先形成 `M0-L4-401` 有限实施方案并取得用户单独授权；没有授权时不得创建执行线程或修改游戏代码。
