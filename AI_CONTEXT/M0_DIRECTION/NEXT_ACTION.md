...
- 当前动作：第八项继续 active。Mac 单机优先、`gpt-5.3-codex-spark` 名称修正和第一层直接统筹已经接受；下一步确认交接单字段、缺失阻塞、逐条验收、`5:4:1` 独立正文统计和最终冻结步骤。
...
未启动：M0-L4-010｜M0-L2-201｜M0-L3-301｜本轮没有创建下层任务
下一步：确认共用交接单字段、缺失阻塞、逐条与抽检范围、5:4:1 独立正文统计，以及整体冻结步骤
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项至第七项 accepted，视觉仍待整体 UI 重做；第八项 active，只有统筹方式局部接受。revision 6 全文源包门禁立即约束未来第二、第三层。第八项接受并由用户最终整体接受后，才可标记 `overall_spec_frozen`；第一层直接统筹不改变这一门禁，也不产生实施授权。
