...
- 当前动作：第一项返工；先并排展示《群星》的短科研卡与用户附件中的完整科技说明，等待用户确定应采用哪种展示层级。
...
当前项：第一项｜玩家先看到的玩法信息与展开顺序｜active_rework
当前对照：《群星》短科研卡｜附件完整科技说明
下一步：用户说明应保留哪种展示层级和阅读体验
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项第一次提案未获接受，必须先完成返工；之后仍需逐项接受描述结构、技术可信度、文体、强度、长度、动态字段与分层交接，并最终整体接受，才可标记 `overall_spec_frozen`。
