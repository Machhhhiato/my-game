...
- 当前动作：第二项返工；让用户复核压短后的技术段，以及机构无自觉地把伤亡转入人力科目并按零故障验收的结尾。
...
当前候选：两句可信技术｜伤亡计入人力损耗｜设备按零故障验收
下一步：用户判断正文语气、技术段长度和地狱笑话强度；确认后再抽取第二项最低标准
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项信息结构 accepted，但视觉明确待整体 UI 重做；第二项 active_rework。正文身份与最低标准确认后，仍需逐项接受数值状态、文体、强度、长度、动态字段与分层交接，并最终整体接受，才可标记 `overall_spec_frozen`。
