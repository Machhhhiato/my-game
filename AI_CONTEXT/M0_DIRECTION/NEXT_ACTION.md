...
- 当前任务：`M0-L4-402-R2`；总部岗位、科研路径骨架、探索、工程选址、整厂生产与事实日志已经拆为独立界面，当前页面保持运行等待现场复查。
- 逻辑主线程 session：`M0-S003` 正在交接；新对话以`你是第一层`建立 `M0-S004` 后接管。
- 强制原话来源：先完整读取 `sessions/20260826-1900-mac-m0-s003-amendment-002.md` 的 `M0-S003-U139` 至 `U143`。摘要、决定表和本文件不能替代原话。
...
- 当前动作：先由新第一层完成接管回执，再继续 402-R2 页面复查。重点核对资源名称、总部建筑岗位、科研路径自动排队、工程选址与既有设施岗位分离、探索归档、整厂生产和日志降噪。当前科技只有两项，属于可验收的交互骨架，不得称为内容完整的科技树。任何新代码修订必须先派发第四层；未获得实现复查接受前不得进入 `M0-L4-403`。
...
当前任务：M0-L4-402-R2｜正式界面交互骨架复查
任务状态：M0-L4-401-R1 accepted｜M0-L4-402-R2 implemented_waiting_user_recheck
...
下一步：新对话输入“你是第一层”，建立 M0-S004；先确认线程门禁与语言门禁，再继续 402-R2 复查
```

## 完成条件

`M0-L4-401-R1` 已由用户试玩接受，`M0-L4-402-R2` 已通过自动和浏览器验证，但实现过程违反线程分工并已纠正规则。当前完成条件是新第一层完成接管并由用户继续现场复查；不自动进入 `M0-L4-403`。
