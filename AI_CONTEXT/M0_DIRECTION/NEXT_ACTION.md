...
- 当前动作：继续在本 session 区分“链路测试通过”和“正式 M0 内容通过”，先确认联调后的最低内容补全门槛，再确认 90 日终点和非目标；不新建会话。
...
3. 继续确认正式 M0 的内容补全门槛、终点和非目标；食物收集与废墟探索已经明确不能被链路占位替代。
4. 只有全部必要内容确认后，才把输出 `SPEC-M0-PLAY-001` 标记为可接受；链路测试通过本身不构成接受。
5. `M0-L1-101` 被接受后，本稳定工作 lane 继续 `M0-L1-102`，但仍需用户单独开始。
