...
action_status: authorized_work_order_pending
...
- 当前任务：`M0-L4-402`；用户已接受 401-R1 并明确要求进入下一阶段，当前先冻结和推送新的执行单。
...
- 当前动作：完成 401 最后一轮资源提示和运行调度令微调，提交推送后为 402 建立新的大型第四层批次；不得进入 `M0-L4-403`。
...
任务状态：M0-L4-401-R1 accepted｜M0-L4-402 authorized_work_order_pending
...
下一步：冻结推送 402 执行单并建立新的第四层批次
```

## 完成条件

`M0-L4-401-R1` 已由用户试玩接受。当前完成条件是把 `M0-L4-402` 的冻结地图、选址、科研与勘测无人机规格整理成单一执行单并派发；不自动进入 `M0-L4-403`。
