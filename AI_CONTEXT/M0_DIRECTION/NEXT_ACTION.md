...
- 当前任务：`M0-L1-104 · 科研、高级资产与前哨复制`，状态为 `ready`。
- 预期输出：`SPEC-M0-PROGRESSION-001`。
- 当前动作：等待用户明确回复 `开始 M0-L1-104`；不得自动启动。
...
预期输出：SPEC-M0-PROGRESSION-001｜尚未创建
下一步：等待用户回复“开始 M0-L1-104”
```

## 完成条件

`M0-L1-103` 已整体 accepted，`M0-L1-104` 依赖满足并处于 ready。只有用户明确回复 `开始 M0-L1-104` 后，才能创建 `SPEC-M0-PROGRESSION-001` 并将任务切到 active。
