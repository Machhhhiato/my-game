...
- 当前动作：第八项第一部分已由 `M0-S003-U080` 接受。`A193` 提出第二部分：缺什么就停在负责它的那一层，只阻塞依赖它的内容，不用假数据遮住；当前等待用户回复`接受第八项第二部分`。
...
当前提案：第八项第二部分｜缺什么停在负责层｜只阻塞依赖项｜禁止假默认值｜等待接受
未启动：M0-L4-005｜M0-L4-010｜M0-L2-201｜M0-L3-301｜本轮没有创建下层任务
下一步：等待用户回复 接受第八项第二部分｜之后才讨论逐条与抽检范围
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项至第七项 accepted，视觉仍待整体 UI 重做；第八项 active，只有统筹方式局部接受。revision 6 全文源包门禁立即约束未来第二、第三层。第八项接受并由用户最终整体接受后，才可标记 `overall_spec_frozen`；冻结后先由第一层派发 `M0-L4-005`。第一层直接统筹不改变冻结门禁，能力审计也不产生工具安装或实施授权。
