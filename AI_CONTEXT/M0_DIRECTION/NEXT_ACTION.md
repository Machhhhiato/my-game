...
- 当前动作：等待用户复核第三项提案：固定解锁留在科技卡，带条件预计放科技侧栏或工程页，真实当前状态放工程与设施页，完成记录保存当时结果。
...
当前项：第三项｜数字、预计效果和当前状态怎样引用｜active_review
...
当前提案：科技卡固定解锁｜科技侧栏带条件预计｜工程与设施页真实当前｜完成报告历史快照
下一步：用户接受或修正第三项；接受后再进入第四项
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项信息结构与第二项两行正文 accepted，视觉仍待整体 UI 重做；第三项 active_review。全文源包门禁立即约束未来第二、第三层。第三至第八项逐项接受并最终整体接受后，才可标记 `overall_spec_frozen`。
