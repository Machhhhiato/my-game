...
用户在 `M0-S003-U087` 至 `U088` 追加并修正后续制作节奏：白天先确定方案、范围和验收；晚上给明确的代码任务按约六小时估算，条件允许时可以运行八小时或更久；次日先验收。详细提案见 `day-night-execution-plan.md`，当前为 review。它是独立的工作协议验收项，不改变本卡的当前产品任务，也不构成实施授权。
...
独立的工作协议验收：用户若接受 `PLAN-M0-DAY-NIGHT-001`，回复 `接受昼夜制作计划`；接受后才把详细规则写入正式协议与未来实现任务包。这不替代 `M0-L4-005` 的验收。
