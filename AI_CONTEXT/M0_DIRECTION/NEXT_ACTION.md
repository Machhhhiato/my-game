...
- 当前动作：开始第三项，讨论固定事实、实时值、预计值与已生效值分别怎样引用；本轮不提前提出第三项方案。
...
当前项：第三项｜数字、预计效果和当前状态怎样引用｜active
...
下一步：讨论固定事实、实时值、预计值与已生效值的显示和文字边界
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项信息结构与第二项两行正文 accepted，视觉仍待整体 UI 重做；第三项 active。全文源包门禁立即约束未来第二、第三层。第三至第八项逐项接受并最终整体接受后，才可标记 `overall_spec_frozen`。
