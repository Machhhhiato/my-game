...
- 当前动作：第七项已经按改进版 accepted；四类科技来源保持有效，旧时代与先驱来源只在首次发现说明；世界内文字不使用机制单位、工程包版本、差异耗时或账本说明，重复工程只写建成，事故只按事实状态记录。权威源包升为 revision 6。下一步开始第八项，确定动态字段、分层交接、逐条验收与最终冻结。
...
下一步：确定第二至第四层分别提供什么、缺失时怎样阻塞、每条文案怎样验收，以及整体何时冻结
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项至第七项 accepted，视觉仍待整体 UI 重做；第八项 active。revision 6 全文源包门禁立即约束未来第二、第三层。第八项接受并由用户最终整体接受后，才可标记 `overall_spec_frozen`。
