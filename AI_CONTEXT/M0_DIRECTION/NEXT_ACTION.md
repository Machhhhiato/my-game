...
- 当前动作：等待用户复核第四项提案：当前情况回原页面，完整经过进日志，状态变化时通知一次；普通进度不通知，无法自动决定的选择才等待玩家。
...
当前项：第四项｜不同内容使用什么载体、什么时候主动提示｜active_review
...
当前提案：当前回原页面｜经过进日志｜状态变化通知一次｜真实选择才等待
下一步：用户接受或修正第四项；接受后再进入第五项
```

## 完成条件

`M0-L1-105` 已在 `ab02d36` accepted，`M0-L1-106` 已 active。第一项信息结构、第二项两行正文和第三项二态展示 accepted，视觉仍待整体 UI 重做；第四项 active_review。全文源包门禁立即约束未来第二、第三层。第四至第八项逐项接受并最终整体接受后，才可标记 `overall_spec_frozen`。
