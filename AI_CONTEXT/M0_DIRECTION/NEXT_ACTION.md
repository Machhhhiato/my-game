...
- 当前任务：`M0-L4-401`；长执行已获授权，正在同步执行单并派发第四层。
...
- 当前动作：提交推送授权记录；只尝试一次新建 `gpt-5.6-terra / medium` 第四层任务，失败立即复用 `01a03800-3d9f-7c70-b1dc-70a30bc8bf01`；随后连续执行、审阅和收口。
...
下一步：授权已确认；同步执行单后立即派发 M0-L4-401 长执行
```

## 完成条件

`M0-L4-401` 已获一次性实施授权。当前完成条件是同步授权、派发第四层、完成代码与验证、提交推送并由第一层交付长执行报告；不自动进入 `M0-L4-402`。
